...
class ConfessionController {
  ...
  createConfession = async (req, res) => {
      ...
      return res
        ...
        .json(
          ResponseHandler.error(
            `Title, body, tagIds and user ID cannot be empty`,
            500,
            null
          )
        );
    ...
    try {
      const confessionId = await this.confessionService.createConfession(
        title,
        body,
        id,
        tagIds
      ...
      res
        .status(200)
        .json(
          ResponseHandler.success(`Confession submitted`, 200, confessionId)
        );
    } catch (err) {
  ...
  getAllTags = async (_, res) => {
    try {
      ...
      res
        .status(200)
        .json(ResponseHandler.success(`All tags retrieved`, 200, allTags));
    } catch (err) {
      res
        .status(400)
  ...
  getPendingConfessions = async (_, res) => {
    try {
      const pendingConfessions =
        await this.confessionService.getPendingConfessions();
      res
        .status(200)
        .json(
          ResponseHandler.success(
            `Pending confessions queried`,
            200,
            pendingConfessions
          )
        );
  ...
  getRejectedConfessions = async (_, res) => {
    try {
      const rejectedConfessions =
        await this.confessionService.getRejectedConfessions();
      res
        .status(200)
        .json(
          ResponseHandler.success(
            `Rejected confession queried`,
            200,
            rejectedConfessions
          )
        );
  ...
  getPublishedConfessions = async (_, res) => {
    try {
      const publishedConfessions =
        await this.confessionService.getPublishedConfessions();
      res
        .status(200)
        .json(
          ResponseHandler.success(
            `Published confessions queried`,
            200,
  ...
  reactConfession = async (req, res) => {
    ...
    try {
      const result = await this.confessionService.reactConfession(
        confessionId,
        reactorId,
        reaction
      );
      res
        .status(200)
        .json(ResponseHandler.success(`React success`, 200, result));
    } catch (err) {
      res
        .status(500)
        .json(ResponseHandler.error(`Something went wrong`, 500, err.message));
  ...
  getConfessions = async (req, res) => {
    ...
    try {
      ...
      res
        .status(200)
        .json(ResponseHandler.success(`Confessions queried`, 200, confessions));
    } catch (err) {
      res
        .status(200)
        .json(ResponseHandler.error(`Something went wrong`, 500, err.message));
  ...
  getConfessionsForUser = async (req, res) => {
    ...
    try {
      const confessions = await this.confessionService.getConfessionsForUser(
        status,
        userId
      );
      res
